...
public class LoadData implements CommandLineRunner {
    ...
    private void createCities() {
        ...
        List<CityData> cityDataList = List.of(
                new CityData("Andijon", "andijon", "/img/andijon.png"),
                new CityData("Buxoro", "buxoro", "/img/buxoro.png"),
                new CityData("Fargona", "fargona", "/img/fargona.png"),
                new CityData("Jizzax", "jizzax", "/img/jizzax.png"),
                new CityData("Namangan", "namangan", "/img/namangan.png"),
                new CityData("Samarkand", "samarkand", "/img/samarkand.png"),
                new CityData("Tashkent", "tashkent", "/img/tashkent.png"));

        List<City> cities = cityDataList.stream()
                .map(data -> new City(null, data.name, data.code, data.image, Collections.emptyList()))
                .toList();
    ...
    private void createRoomFacilities() {
        ...
        List<RoomFacility> roomFacilities = facilities.stream()
                .map(name -> new RoomFacility(null, name))
                .toList();
    ...
    private void createUsers() {
        ...
        Role managerRole = roleRepository.findByName("MANAGER")
                .orElseThrow(() -> new EntityNotFoundException("Role", "MANAGER"));

        List<User> users = List.of(
                new User(null, "admin", passwordEncoder.encode("admin123"),
                        "admin@booking.uz", List.of(adminRole)),
                new User(null, "asror", passwordEncoder.encode("123"),
                        "asror@gmail.com", List.of(adminRole)),
                new User(null, "manager1", passwordEncoder.encode("manager123"),
                        "manager@booking.uz", List.of(managerRole)),
                new User(null, "john_doe", passwordEncoder.encode("user123"),
                        "john@example.com", List.of(userRole)),
                new User(null, "jane_smith", passwordEncoder.encode("user123"),
                        "jane@example.com", List.of(userRole)),
                new User(null, "mike_wilson", passwordEncoder.encode("user123"),
                        "mike@example.com", List.of(userRole)),
                new User(null, "sarah_johnson", passwordEncoder.encode("user123"),
                        "sarah@example.com", List.of(userRole)),
                new User(null, "david_brown", passwordEncoder.encode("user123"),
        ...
        for (Hotel hotel : hotels) {
            ...
            List<RoomTypeConfig> configs = List.of(
                    ...
                    new RoomTypeConfig("Executive Suite", 3, 250_000L, 2,
                            List.of(new BedConfig("King", 2), new BedConfig("Sofa Bed", 1))));

            for (RoomTypeConfig config : configs) {
                RoomType roomType = new RoomType(null, hotel.getName() + " " + config.name,
                ...
                List<Bed> beds = config.beds.stream()
                        .map(bedConfig -> new Bed(null, bedConfig.type, bedConfig.count, roomType))
                        .toList();
                allBeds.addAll(beds);
                ...
                for (int i = 1; i <= config.roomCount; i++) {
                    allRooms.add(new Room(null, i, roomType, hotel));
                }
            }
        }
    ...
    private Map<String, HotelFacility> createFacilitiesMap() {
        ...
        List<FacilityData> facilityDataList = List.of(
                new FacilityData("wifi", "Free WI-FI", "/img/wi-fi.png"),
                new FacilityData("parking", "Free Parking", "/img/parking.png"),
                new FacilityData("pool", "Swimming Pool", "/img/pool.png"),
                new FacilityData("breakfast", "Breakfast Included", "/img/breakfast.png"),
                new FacilityData("spa", "Spa & Wellness Center", "/img/spa.png"),
                new FacilityData("gym", "Fitness Center", "/img/gym.png"),
                new FacilityData("bar", "Bar & Lounge", "/img/bar.png"),
                new FacilityData("restaurant", "Restaurant", "/img/restaurant.png"),
                new FacilityData("kidsClub", "Kids Club", "/img/kids.png"),
                new FacilityData("shuttle", "Airport Shuttle", "/img/shuttle.png"),
                new FacilityData("conference", "Conference Hall", "/img/conference.png"),
                new FacilityData("laundry", "Laundry Service", "/img/laundry.png"),
                new FacilityData("concierge", "24/7 Concierge", "/img/concierge.png"));

        facilityDataList.forEach(data -> facilities.put(data.key, new HotelFacility(null, data.name, data.icon)));
